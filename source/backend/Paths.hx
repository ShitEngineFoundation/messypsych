...
class Paths
{
	...
	public static function returnSound(key:String, ?path:String, ?modsAllowed:Bool = true, ?beepOnNull:Bool = true, ?stream:Bool = false)
	{
		...
		var replacementSound:Sound = null;
		var existsOpenFL = OpenFlAssets.exists(file, SOUND);
		var existsSys = #if sys FileSystem.exists(file) #else false #end;
		if (canStreamSound(file) && stream)
		{
			if (existsOpenFL)
				replacementSound = FlxG.assets.streamSound(file);
			else if (existsSys)
				replacementSound = getMusicStream(file);
		}
		if (!currentTrackedSounds.exists(file))
		{
			#if sys
			if (FileSystem.exists(file))
				currentTrackedSounds.set(file, replacementSound ?? Sound.fromFile(file));
			#else
			if (OpenFlAssets.exists(file, SOUND))
				currentTrackedSounds.set(file, replacementSound ?? OpenFlAssets.getSound(file));
			#end
		else if (beepOnNull)
		{
	...
	public static function canStreamSound(id:String):Bool
	{
		#if lime_vorbis
		// Check if file is really OGG/Vorbis
		final vorbis = lime.media.vorbis.VorbisFile.fromFile((id));
		if (vorbis != null)
		{
			vorbis.clear();
	...
	public static function getMusicStream(id:String, useCache:Bool = true):Sound
	{
		...
		else
		{
			...
			return Sound.fromFile(id);
		}
		#else
		// TODO: Streaming sound
		return Sound.fromFIle(id, useCache);
		#end
	}
